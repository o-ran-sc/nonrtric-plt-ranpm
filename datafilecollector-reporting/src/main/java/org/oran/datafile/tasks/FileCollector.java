...
public class FileCollector {
    ...
    private Mono<Optional<FilePublishInformation>> tryCollectFile(FileData fileData) {
        logger.trace("starting to collectFile {}", fileData.fileInfo.fileName());
        ...
        try (FileCollectClient currentClient = createClient(fileData)) {
            ...
            logger.warn("Failed to download file, not retryable: {} {}, reason: {}", fileData.sourceName(),
                fileData.fileInfo.fileName(), nre.getMessage());
            incFailedAttemptsCounter(fileData);
            return Mono.just(Optional.empty()); // Give up
        } catch (DatafileTaskException e) {
            logger.warn("Failed to download file: {} {}, reason: {}", fileData.sourceName(), fileData.fileInfo.fileName(),
                e.getMessage());
            incFailedAttemptsCounter(fileData);
            return Mono.error(e);
        } catch (Exception throwable) {
            logger.warn("Failed to close client: {} {}, reason: {}", fileData.sourceName(), fileData.fileInfo.fileName(),
    ...
    public FilePublishInformation createFilePublishInformation(FileData fileData) {
        ...
        return FilePublishInformation.builder() //
            ...
            .name(metaData.sourceName + "/" + fileData.fileInfo.fileName()) //
            .compression(fileData.fileInfo.fileCompression) //
            .fileFormatType(fileData.fileInfo.fileFormat) //
            .fileFormatVersion(fileData.fileInfo.fileDataType) //
