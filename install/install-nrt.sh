...
# --short has been marked for deprecation.
# Once it is deprecated "--short" can be removed as "kubectl version" should give same result after deprecation
export KUBERNETESVERSION=$(kubectl version --short 2>/dev/null | grep 'Server Version' | awk '{print $3}')
if [ $? -ne 0 ]; then
